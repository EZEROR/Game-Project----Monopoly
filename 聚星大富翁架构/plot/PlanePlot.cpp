...
PlanePlot::PlanePlot(int x, int y, int w, int h) :Plot(x, y, w, h)
{
	this->price_[0] = 250; // 数组里存储不同等级城市地块的价格
	this->price_[1] = 250;
	this->price_[2] = 250;
	this->name_ = "飞机";
...
void PlanePlot::Draw()
{
	...
	if (owner_ID_ == 0) {
		//显示不同图片
		loadimage(&image, _T("asserts/PlanePlot_write.png"));
	}
	else if (owner_ID_ == 1) {
		loadimage(&image, _T("asserts/PlanePlot_red.png"), 47, 52);
	}
	else if (owner_ID_ == 2) {
		loadimage(&image, _T("asserts/PlanePlot_yellow.png"));
	}
	else if (owner_ID_ == 3) {
		loadimage(&image, _T("asserts/PlanePlot_blue.png"));
	}
	else
	{
		loadimage(&image, _T("asserts/PlanePlot_green.png"), 47, 52);
	}
...
//参数:无
//返回值:NULL
void PlanePlot::DrawWindow(Player& player)
{
	...
	if (owner_ID_ == 0)
	{
		//弹窗背景
		IMAGE image;
		loadimage(&image, "asserts/PlanePlotwindow.png", 207, 202);
...
*/
//判定 
bool PlanePlot::inWindow(const ExMessage& msg, Player& player)//0未点击，1点击了是,2点击退出
{
